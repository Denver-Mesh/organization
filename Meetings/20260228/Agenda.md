This document is a WORKING DRAFT until finalized. Input and adjustments are welcome.

This document will be finalized and shared no later than **48 hours prior** to the scheduled meeting.

# Agenda for 02/28/2026 Town Hall Meeting

**Total estimated time:** 82 minutes

## 1. Welcome and Introductions (5 minutes)

- Brief welcome message from host
- Introduction of admin team
  - Clarification of how appointments were made following previous meeting
  - Clarification that current admins do not necessarily constitute potential organization board members

## 2. Review of Previous Meeting Minutes (5 minutes)

- Summary of key points from the [previous meeting](../20260207/Meeting-Minutes-02072026.pdf)
- Discussion of any outstanding action items

## 3. Acknowledgements (5 minutes)

- Highlighting of recent achievements and contributions by members
  - `M3SHGHØST`'s deployment of a MeshCore repeater on Mount Morrison
  - `NVON`'s native Mac client for Meshtastic
  - `dude.eth` Reticulum client
  - `omgitsgela`'s deployment of a MeshCore repeater on Lookout Mountain
  - `Zeva` helping craft v2.0 of the MeshCore naming scheme with airport codes
- Acknowledge other organization existence
- Ryan Larkin Invitation Adventure Race - great proving ground for Meshtastic: https://www.rliar.org/

## 4. Meeting Purpose (2 minutes)

- Clarification of the purpose of the meeting
  - To provide updates on organizational activities
  - To discuss and plan future initiatives
  - To address any questions or concerns from members
  - Topics NOT planned for discussion:
    - Node placements
    - Technical discussions regarding network architecture
    - Specifics regarding the DenverMesh website

## 5. Organizational Updates (5 minutes)

- Overview of recent activities and developments within the organization
  - Update on GitHub Sponsors program and donation management
  - Update on ongoing projects and initiatives
  - Update on community engagement efforts

## 6. Organizational Goals (15 minutes)

- Field ideas and suggestions for organizational goals
  - Ottawa Mesh as an example: https://ottawamesh.ca/community/goals/
  - Short-term goals (next 6 months)
  - Long-term goals (next 1-3 years)
  - How to we measure success in achieving these goals? (metrics, milestones, etc.)
- Discussion of how to prioritize and achieve these goals

## 7. Organizational Structure and Governance (15 minutes)

- Field ideas and suggestions for organizational structure and governance
  - Potential board member roles and responsibilities
    - How would those roles be filled? (elections, appointments, etc.)
    - Term lengths and limits for board members
  - Decision-making processes
    - When/how often would the board meet?
    - How would decisions be made (majority vote, general election, board consensus, etc.)?
      - What constitutes a "decision" that would require board input vs. something that could be handled by admins or volunteers?
      - What constitutes a "majority" for voting purposes? (simple majority, supermajority, etc.)
      - What constitutes a "general election" for voting purposes? (all members, only active members, etc.)
  - Communication channels and protocols
    - Discussion regarding future Discord usage will be reserved for a later agenda item
    
## 8. Solicitation of Volunteers or Nominations for Board Positions (10 minutes)

- Solicit volunteers to form committee(s) to further develop organizational goals and structure
  - Education
  - Guidance for Network modes
  - Node modes suggestions (repeater, client, etc)
  - Event participation & Volunteering
- How does everyone feel about a non-profit or other organizaitonal role
  - How likely are members to donate?  Frequency? Amount?
## 9. Website Upgrades & Changes (5 minutes)
- Regional portals / webpages with Ops
- Mapping software updates and integration with backend server
- Updates for Meshcore & Reticulum
- Content types & Editing
  
## 10. Open Floor for Questions and Discussion (20 minutes)

- Open the floor for any questions, comments, or concerns from members
  - Questions regarding Discord usage will be allowed here
    - No decision is expected to be made regarding Discord usage during this meeting, but feedback will be collected and considered for future discussions
